        ...
        <div class="grid row">
            @foreach ($places as $place)
                <article id="place-{{ $place->id }}" class="place grid-item col-xs-12 col-sm-6 col-md-4 col-lg-3">
                    <img class="featured-image" src="{{ $place->getImage()->getThumbUrl() }}" />
                    <h3 class="title"><a href="{{ url('/place/' . $place->id) }}">
                        {{ $place->getTranslation()->name }}</a></h3>
                    <div class="content">{!! $place->getTranslation()->content !!}</div>
